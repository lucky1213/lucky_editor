...
part 'nodes/embeddable.dart';
part 'nodes/leaf.dart';
part 'nodes/line.dart';
part 'nodes/node.dart';

part '../rules/delete.dart';
part '../rules/format.dart';
part '../rules/insert.dart';
part '../rules/rule.dart';

part '../quill_delta.dart';
...
class Document {
  ...
  static Delta _transform(Delta delta) {
    ...
    for (var i = 0; i < ops.length; i++) {
      final op = ops[i];
      res.push(op);
      _autoAppendNewlineAfterEmbeddable(i, ops, op, res);
    }
    return res;
  }
  ...
  static void _autoAppendNewlineAfterEmbeddable(
    ...
    final nextOpIsEmbed = i + 1 < ops.length &&
        ...
        (type != null
            ...
            : ((ops[i + 1].data as Map).containsKey(BlockEmbed.imageType) ||
                (ops[i + 1].data as Map).containsKey(BlockEmbed.videoType)));
    if (nextOpIsEmbed &&
        op.data is String &&
    ...
    final opInsertEmbed = op.isInsert &&
        op.data is Map &&
        (type != null
            ? (op.data as Map).containsKey(type)
