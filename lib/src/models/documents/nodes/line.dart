...
///
/// Line serves as a container for [Leaf]s, like [TextLeaf] and [EmbedLeaf].
///
/// When a line contains an embed, it fully occupies the line, no other embeds
/// or text nodes are allowed.
class Line extends ContainerNode<Leaf?> {
  @override
  ...
  bool get hasEmbed {
    return children.any((child) => child is EmbedLeaf);
  }
  ...
  String getPlainText(int offset, int len) {
    ...
    if (res.length == 1) {
      ...
      return text == EmbedLeaf.kObjectReplacementCharacter
          ? ''
    ...
    if (text != EmbedLeaf.kObjectReplacementCharacter) {
      total.add(text.substring(text.length - firstNodeLen));
    }

    for (var i = 1; i < res.length - 1; i++) {
      if (res[i].item2 != EmbedLeaf.kObjectReplacementCharacter) {
