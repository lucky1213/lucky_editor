...
class RealSelection {
  const RealSelection(
      {this.start = const RealPosition(), this.end = const RealPosition()});
  final RealPosition start;
  final RealPosition end;

  bool get isValid => start.line != -1 && end.line != -1;

  bool get isCorssLine => isValid && start.line != end.line;

  static RealSelection get invalid => const RealSelection();

  RealSelection setLine(int lineNumber) {
    List<int> list = [start.line, end.line];
    if (!list.contains(lineNumber)) {
      if (start.line == -1 && end.line == -1) {
        list = [lineNumber, lineNumber];
      } else {
        // 往下一行选中的
        if (lineNumber > list.last) {
          list
            ..add(lineNumber)
            ..remove(-1)
            ..sort();
      ...
      return RealSelection(
        start: RealPosition(line: list.first),
        end: RealPosition(line: list.last),
      );
    }
    return this;
  }

  RealSelection setPosition(int position) {
    List<int> list = [start.position, end.position];
    ...
    return RealSelection(
      start: start.copyWith(position: position),
      end: end.copyWith(position: position),
    );
  }
...
class RealPosition {
  const RealPosition({this.line = -1, this.position = -1});
  final int line;
  final int position;

  static RealPosition get invalid => const RealPosition();

  RealPosition copyWith({
    int? line,
    int? position,
  }) {
    return RealPosition(
...
class ExtendedRenderEditable extends RenderBox
  ...
  /// [ExtendedRenderEditable]'s text content.
  ///
  ...
  void _updateSelectionExtentsVisibility(Offset effectiveOffset) {
    ...
    final Offset startOffset = getOffsetForCaret(
  ...
  set selection(TextSelection? value) {
    ...
    if (!value!.isCollapsed) {
      _realSelection = RealSelection(
        start: _realSelection.start.copyWith(
          position: value.baseOffset,
        ),
        end: _realSelection.end.copyWith(
          position: value.extentOffset,
        ),
      );
    } else {
      _realSelection = RealSelection(
        start: RealPosition(line: _currentLine, position: value.extentOffset),
        end: RealPosition(line: _currentLine, position: value.extentOffset),
      );
    }
    print(_realSelection);
    _selection = value;
  ...
  ///
  /// [ExtendedRenderEditable] is responsible for calculating the [Offset] of this
  ...
  ///
  /// Since [ExtendedRenderEditable] does not handle selection manipulation
  ...
  bool _onlyWhitespace(TextRange range) {
    for (int i = range.start; i < range.end; i++) {
      final int codeUnit = _currentLinePlainText.codeUnitAt(i);
  ...
  List<TextSelectionPoint> getEndpointsForSelection(TextSelection selection) {
    ...
    final RenderParagraph renderParagraph = getRenderParagraphForParent();
    final List<ui.TextBox> boxes = selection.isCollapsed
        ? <ui.TextBox>[]
        : renderParagraph.getBoxesForSelection(selection,
            boxHeightStyle: selectionHeightStyle,
            boxWidthStyle: selectionWidthStyle);
    if (boxes.isEmpty) {
      // TODO(mpcomplete): This doesn't work well at an RTL/LTR boundary.
      final Offset caretOffset =
          getOffsetForCaret(selection.extent, _caretPrototype);
      final Offset start =
          Offset(0.0, renderParagraph.textPainter.preferredLineHeight) +
              caretOffset +
              paintOffset;
      return <TextSelectionPoint>[TextSelectionPoint(start, null)];
    } else {
      final Offset start = Offset(
              boxes.first.start.clamp(0, renderParagraph.size.width),
              boxes.first.bottom) +
          globalToLocal(renderParagraph.localToGlobal(Offset.zero)) +
          paintOffset;
      final Offset end = Offset(
              boxes.last.end.clamp(0, renderParagraph.size.width),
              boxes.last.bottom) +
          globalToLocal(renderParagraph.localToGlobal(Offset.zero)) +
          paintOffset;
  ...
  Rect getLocalRectForCaret(TextPosition caretPosition) {
    _computeTextMetricsIfNeeded();
    final Offset caretOffset =
        getOffsetForCaret(caretPosition, _caretPrototype);
  ...
  // TextPosition _linePosition = TextPosition(offset: 0);

  RealSelection _realSelection = RealSelection.invalid;
  int _crossLine = -1;
  int _currentLine = 0;

  // RenderParagraph _line

  List<RenderBox> get _renderBoxes => getChildrenAsList();

  String get _currentLinePlainText {
    return getRenderParagraphForParent()
        .text
        .toPlainText(includeSemanticsLabels: false);
  }

  // TextPosition getRealTextPosition(int virtual) {
  //   for (var i = _linePosition.offset - 1; i >= 0; i--) {
  //     virtual += getRenderParagraphForParent(_renderBoxes[i])
  //         .text
  //         .toPlainText(includeSemanticsLabels: false)
  //         .length;
  //   }
  //   return TextPosition(offset: virtual);
  // }

  int getLinePositionForOffset(Offset offset, [bool record = true]) {
    int lineNumber =
        TextPosition(offset: childCount == 0 ? 0 : childCount - 1).offset;

    for (var i = 0; i < childCount; i++) {
      final e = _renderBoxes[i];
      final double dy =
          globalToLocal(e.localToGlobal(Offset(0.0, e.size.height))).dy;
      if (offset.dy < dy) {
        lineNumber = i;
        break;
      }
    ...
    if (record) {
      ...
      _realSelection = _realSelection.setLine(lineNumber);
    }
    return lineNumber;
  }

  RenderParagraph getRenderParagraphForOffset(Offset offset,
      [bool record = true]) {
    getLinePositionForOffset(offset, record);
    return getRenderParagraphForParent();
  }

  // TextPosition getCrossLinePositionForOffset(
  //     Offset offset, _TextSelectionHandlePosition handle) {
  //   final position = getPositionForOffset(offset);
  //   if (handle == _TextSelectionHandlePosition.start) {
  //     _crossLineSelection = _crossLineSelection.withItem1(position.offset);
  //   } else {
  //     _crossLineSelection = _crossLineSelection.withItem2(position.offset);
  //   }
  //   return position;
  // }
  ...
  TextPosition getPositionForOffset(Offset offset) {
    // 获取了end
    final position = getRenderParagraphForOffset(offset)
        .getPositionForOffset(globalToLocal(offset));
    if (_realSelection.isCorssLine) {
      _realSelection = _realSelection.setPosition(position.offset);
    }

    // List<int> list = [_realSelection.item1.line, _realSelection.item2.line];
    //   if (!list.contains(lineNumber)) {
    //     if (_realSelection.item1.line == -1 &&
    //         _realSelection.item1.line == -1) {
    //       list = [lineNumber, lineNumber];
    //     } else {
    //       list
    //         ..add(lineNumber)
    ...
    //         ..sort();
    //     }
    //     _realSelection = Tuple2(
    //         RealSelection(line: list.first), RealSelection(line: list.last));
    //   }
    // // 获取start
    // if (_crossRange.item1 != _crossRange.item2) {
    //   print('需要取第一行选中的position');
    //   final position =
    //       getRenderParagraphForParent(_renderBoxes[_crossRange.item1])
    //           .getPositionForOffset(globalToLocal(offset));
    // }
    return position;
  }
  ...
  @override
  TextRange getWordBoundary(TextPosition position) {
    return getRenderParagraphForParent().getWordBoundary(position);
  }

  Offset getOffsetForCaret(TextPosition position, Rect rect) {
    final RenderParagraph renderParagraph = getRenderParagraphForParent();
    final Offset offset =
        globalToLocal(renderParagraph.localToGlobal(Offset.zero));
    return offset + renderParagraph.getOffsetForCaret(position, rect);
  }

  double? getFullHeightForCaret(TextPosition position, Rect rect) {
    final RenderParagraph renderParagraph = getRenderParagraphForParent();
    return renderParagraph.textPainter.getFullHeightForCaret(position, rect);
  }

  RenderParagraph getRenderParagraphForParent([RenderBox? box]) {
    box ??= _renderBoxes[_currentLine];
    switch (box.runtimeType) {
      case RenderSemanticsAnnotations:
        return getRenderParagraphForParent(
            (box as RenderSemanticsAnnotations).child!);
      case RenderPadding:
        return getRenderParagraphForParent((box as RenderPadding).child!);
      case RenderConstrainedBox:
        return getRenderParagraphForParent(
            (box as RenderConstrainedBox).child!);
      case RenderDecoratedBox:
        return getRenderParagraphForParent((box as RenderDecoratedBox).child!);
      default:
  ...
  TextSelection _getWordAtOffset(TextPosition position) {
    ...
    if (obscureText) {
      ...
      // the position.
            ...
            _currentLinePlainText.codeUnitAt(position.offset)) &&
        position.offset > 0) {
...
/// An interface that paints within a [ExtendedRenderEditable]'s bounds, above or
...
/// blocks). It can paint independently from its [ExtendedRenderEditable], allowing it
/// to repaint without triggering a repaint on the entire [ExtendedRenderEditable] stack
/// when only auxiliary content changes (e.g. a blinking cursor) are present. It
/// will be scheduled to repaint when:
///
///  * It's assigned to a new [ExtendedRenderEditable] and the [shouldRepaint] method
///    returns true.
///  * Any of the [ExtendedRenderEditable]s it is attached to repaints.
///  * The [notifyListeners] method is called, which typically happens when the
///    painter's attributes change.
///
/// See also:
///
///  * [ExtendedRenderEditable.foregroundPainter], which takes a [RenderEditablePainter]
///    and sets it as the foreground painter of the [RenderEditable].
///  * [ExtendedRenderEditable.painter], which takes a [RenderEditablePainter]
...
abstract class RenderEditablePainter extends ChangeNotifier {
  ...
  /// is provided to a [ExtendedRenderEditable].
  ///
  ...
  /// [ExtendedRenderEditable]s it attaches to repaint, even if [shouldRepaint] returns
  /// false.
  bool shouldRepaint(RenderEditablePainter? oldDelegate);

  /// Paints within the bounds of a [ExtendedRenderEditable].
  ///
  /// The given [Canvas] has the same coordinate space as the [ExtendedRenderEditable],
  /// which may be different from the coordinate space the [ExtendedRenderEditable]'s
  /// [TextPainter] uses, when the text moves inside the [ExtendedRenderEditable].
  ///
  /// Paint operations performed outside of the region defined by the [canvas]'s
  /// origin and the [size] parameter may get clipped, when [ExtendedRenderEditable]'s
  /// [ExtendedRenderEditable.clipBehavior] is not [Clip.none].
  void paint(Canvas canvas, Size size, ExtendedRenderEditable renderEditable);
}
...
class _TextHighlightPainter extends RenderEditablePainter {
  ...
  void paint(Canvas canvas, Size size, ExtendedRenderEditable renderEditable) {
    ...
    final RenderParagraph renderParagraph =
        renderEditable.getRenderParagraphForParent();

    final RenderParagraph startRenderParagraph =
        renderEditable.getRenderParagraphForParent(renderEditable
            ._renderBoxes[renderEditable._realSelection.start.line]);
    final RenderParagraph endRenderParagraph =
        renderEditable.getRenderParagraphForParent(renderEditable
            ._renderBoxes[renderEditable._realSelection.end.line]);
    final int start = renderEditable._realSelection.start.position;
    int end = renderEditable._realSelection.end.position;
    if (renderEditable._realSelection.isCorssLine) {
      end = startRenderParagraph.text
          .toPlainText(includeSemanticsLabels: false)
          .length;
    }

    final List<TextBox> boxes = startRenderParagraph.getBoxesForSelection(
      TextSelection(
        baseOffset: start,
        extentOffset: end,
      ),
      ...
      canvas.drawRect(
          box.toRect().shift(renderEditable._paintOffset +
              renderEditable.globalToLocal(
                  startRenderParagraph.localToGlobal(Offset.zero))),
          highlightPaint);
    }

    if (renderEditable._realSelection.isCorssLine) {
      end = renderEditable._realSelection.end.position;
      final List<TextBox> boxes = endRenderParagraph.getBoxesForSelection(
      ...
      for (final TextBox box in boxes) {
        canvas.drawRect(
            box.toRect().shift(renderEditable._paintOffset +
                renderEditable.globalToLocal(
                    endRenderParagraph.localToGlobal(Offset.zero))),
            highlightPaint);
      }
    }
    ...
    //   box
    //       .toRect()
    //       .inflate(renderEditable
    //           .globalToLocal(renderParagraph.localToGlobal(Offset.zero))
    //           .dy)
    //       .shift(renderEditable._paintOffset)
    //       .intersect(Rect.fromLTWH(0, 0, renderParagraph.textPainter.width,
    //           renderParagraph.textPainter.height)),
    //   highlightPaint,
    // );
  }
...
class _FloatingCursorPainter extends RenderEditablePainter {
  ...
  void paintRegularCursor(Canvas canvas, ExtendedRenderEditable renderEditable,
    ...
    final double? caretHeight =
        renderEditable.getFullHeightForCaret(textPosition, caretPrototype);
    if (caretHeight != null) {
      switch (defaultTargetPlatform) {
        case TargetPlatform.iOS:
        case TargetPlatform.macOS:
